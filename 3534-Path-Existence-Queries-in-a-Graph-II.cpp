...
public:
    vector<int> pathExistenceQueries(int n, vector<int>& nums, int maxDiff, vector<vector<int>>& queries) {
        ...
        int mxL = bit_width((unsigned)n)+1;

        for(int i=0;i<n;i++){
            xId[i] = {nums[i], i};
        }
        sort(xId, xId+n);
        for(int i=0;i<n;i++) //pos of idx of sorted xId
            pos[xId[i].second] = i;

        //sliding window
        for(int l=0,r=0;l<n;l++){
            while(r+1<n and xId[r+1].first-xId[l].first<=maxDiff) 
                r++;
            up[0][l] = r;
        }
...
private:
    static int cnt(int u, int v, int L){
        ...
        if(up[L-1][u]<v) return -1; //can not reach v
        int step = 0;
        for(int j=L-1;j>=0;j--){
